...
describe('Time Validation & Correction Mode', function()
    ...
    describe('detectErroneousEntries', function()
        ...
        it('should detect entries with negative durations', function()
            ...
            local entries =
                config_module.obj.content.data['2023']['11']['Wednesday']['TestProject']['test.lua'].items
            ...
            for _, issue in ipairs(validation_results.errors[1].issues) do
                if
                    string.find(issue, 'Startzeit nach Endzeit') or string.find(issue, 'Negative')
                then
            ...
            assert.is_true(
                found_time_error,
                'Should detect start time after end time or negative duration'
            )
        end)
