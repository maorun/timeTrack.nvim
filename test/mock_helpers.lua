...
function notify_mock:get_message() return self.message end
function notify_mock:get_level() return self.level end
function notify_mock:get_opts() return self.opts end
function notify_mock:get_all_calls() return self.all_calls end
...
function M.reset_all_was_called_flags()
    if M.input_mock.was_called_count then M.input_mock.was_called_count = 0 end -- Specific to input mock
    if M.select_mock.was_called then M.select_mock.was_called = false end
    if M.weekday_select_mock.was_called then M.weekday_select_mock.was_called = false end
    if M.notify_mock.was_called then M.notify_mock.was_called = false end
    if M.notify_mock.all_calls then M.notify_mock.all_calls = {} end
end
