...
describe("Project and File Tracking Functionality", function()
    ...
    local function get_mock_time(offset_seconds)
        ...
        return os.time({
            year = tonumber(mock_date_params.year), month = mock_date_params.month_num, day = mock_date_params.day_of_month,
            hour = mock_date_params.hour, min = mock_date_params.min, sec = mock_date_params.sec
        }) + offset_seconds
    end
    ...
    setup(function()
        ...
        os.date = function(format, time_val)
            if format == "%Y" then return mock_date_params.year end
            if format == "%W" then return mock_date_params.week end
            if format == "%A" then return mock_date_params.weekday_name end
            if format == "*t" then
                ...
                if time_val == nil or (t.year == tonumber(mock_date_params.year) and t.month == mock_date_params.month_num and t.day == mock_date_params.day_of_month) then
    ...
    describe("TimeStart and TimeStop", function()
        it("should create entries in the specified project and file", function()
            ...
            time_init_module.TimeStart({ project = project, file = file, time = startTime, weekday = "Monday" })
            time_init_module.TimeStop({ project = project, file = file, time = stopTime, weekday = "Monday" })
        ...
        it("should default to 'default_project' and 'default_file' if not specified", function()
            ...
            local data = get_data_from_json()
            local day_data = data.data[mock_date_params.year][mock_date_params.week]["default_project"]["default_file"].weekdays["Monday"]
    ...
    describe("addTime", function()
        it("should add time to a specific project/file", function()
            time_init_module.addTime({ time = 2.5, weekday = "Tuesday", project = "ProjectX", file = "file_alpha.md" })
            local data = get_data_from_json()
            local day_items = data.data[mock_date_params.year][mock_date_params.week]["ProjectX"]["file_alpha.md"].weekdays["Tuesday"].items
        ...
        it("should use default project/file if not specified", function()
            ...
            local data = get_data_from_json()
            local day_items = data.data[mock_date_params.year][mock_date_params.week]["default_project"]["default_file"].weekdays["Wednesday"].items
            assert.are.same(1, #day_items)
    ...
    describe("subtractTime", function()
        ...
        it("should add a time entry (like addTime) to a specific project/file", function()
            time_init_module.subtractTime({ time = 1, weekday = "Thursday", project = "ProjectY", file = "file_beta.txt" })
            local data = get_data_from_json()
            local day_items = data.data[mock_date_params.year][mock_date_params.week]["ProjectY"]["file_beta.txt"].weekdays["Thursday"].items
            assert.are.same(1, #day_items)
    ...
    describe("setTime", function()
        it("should set time for a specific project/file, clearing previous entries", function()
            -- Add initial entry
            time_init_module.addTime({ time = 1, weekday = "Friday", project = "ProjectZ", file = "file_gamma.py" })
            time_init_module.addTime({ time = 2, weekday = "Friday", project = "ProjectZ", file = "file_gamma.py" })

            -- Now setTime
            time_init_module.setTime({ time = 3.5, weekday = "Friday", project = "ProjectZ", file = "file_gamma.py" })
            local data = get_data_from_json()
            local day_items = data.data[mock_date_params.year][mock_date_params.week]["ProjectZ"]["file_gamma.py"].weekdays["Friday"].items
    ...
    describe("calculate", function()
        it("should correctly calculate summaries across projects, files, and weekdays", function()
            ...
            time_init_module.addTime({ time = 2, weekday = "Monday", project = "Alpha", file = "main.lua" })  -- Alpha/main.lua/Monday: 2hr. Overhour: 2-8 = -6
            time_init_module.addTime({ time = 3, weekday = "Monday", project = "Alpha", file = "utils.lua" }) -- Alpha/utils.lua/Monday: 3hr. Overhour: 3-8 = -5
            time_init_module.addTime({ time = 4, weekday = "Tuesday", project = "Alpha", file = "main.lua" }) -- Alpha/main.lua/Tuesday: 4hr. Overhour: 4-8 = -4
            time_init_module.addTime({ time = 1, weekday = "Monday", project = "Bravo", file = "init.lua" })  -- Bravo/init.lua/Monday: 1hr. Overhour: 1-8 = -7

            time_init_module.calculate({ year = mock_date_params.year, weeknumber = mock_date_params.week })
            local data = get_data_from_json()
            local week_summary = data.data[mock_date_params.year][mock_date_params.week].summary
            local alpha_main_mon = data.data[mock_date_params.year][mock_date_params.week]["Alpha"]["main.lua"].weekdays["Monday"].summary
            local alpha_utils_mon = data.data[mock_date_params.year][mock_date_params.week]["Alpha"]["utils.lua"].weekdays["Monday"].summary
            local alpha_main_tue = data.data[mock_date_params.year][mock_date_params.week]["Alpha"]["main.lua"].weekdays["Tuesday"].summary
            local bravo_init_mon = data.data[mock_date_params.year][mock_date_params.week]["Bravo"]["init.lua"].weekdays["Monday"].summary
            ...
            assert.is_near(-22, week_summary.overhour, 0.001, "Week summary overhour incorrect. Got: " .. inspect(week_summary.overhour))
        end)
    end)
end)
