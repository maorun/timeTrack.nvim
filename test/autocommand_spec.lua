...
describe('Autocommand BufEnter Tests', function()
    ...
    before_each(function()
        ...
        if not helpers.mock_os_time then -- Basic mock if not in helper
            ...
            helpers.set_mock_os_time = function(t)
                current_time = t
            end
        else
        ...
        if not helpers.mock_nvim_buf_get_name then -- Basic mock
            ...
            helpers.set_mock_nvim_buf_get_name = function(name)
                current_buf_name = name
            end
        else
        ...
        if not helpers.mock_nvim_get_current_buf then -- Basic mock
            ...
            vim.api.nvim_get_current_buf = function()
                return current_buf_handle
            end
            helpers.set_mock_nvim_get_current_buf = function(handle)
                current_buf_handle = handle
            end
        else
    ...
    it(
        '6. BufEnter for path ending in slash (directory-like): TimeStart with parent as project',
        function()
            local buf_handle = 60
