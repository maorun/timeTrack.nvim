...
describe("maorun.time.ui.select", function()
  ...
  it("should handle default options (all true)", function()
    ...
    assert.are_equal("Select weekday:", helper.weekday_select_mock:get_show_called_with_options().prompt)
  ...
  it("should handle opts.hours = false", function()
    ...
    assert.are_equal(2, helper.input_mock:get_call_count(), "vim.ui.input should be called twice for project and file")
    assert.is_true(helper.weekday_select_mock:was_called_flag())
  end)
  ...
  it("should handle opts.weekday = false (and hours = true, causing warning)", function()
    ...
    assert.are_equal("Weekday is required when hours are enabled.", helper.notify_mock:get_message())
    assert.are_equal(vim.log.levels.WARN, helper.notify_mock:get_level())
    assert.is_false(helper.weekday_select_mock:was_called_flag(), "Weekday select should not be called")
    ...
    assert.are_equal(2, helper.input_mock:get_call_count(), "Input should be called for project and file only")
  end)
  ...
  describe("input validation for hours", function()
    local function test_invalid_hours(hours_input_val, test_label, expected_notify_msg)
      ...
      assert.is_true(helper.notify_mock:was_called_flag(), "Notify should be called for " .. test_label)
      assert.are_equal(expected_notify_msg, helper.notify_mock:get_message())
      assert.are_equal(vim.log.levels.WARN, helper.notify_mock:get_level())
      assert.are_equal(3, helper.input_mock:get_call_count(), "Input should be called thrice for " .. test_label)
      assert.is_true(helper.weekday_select_mock:was_called_flag(), "Weekday select should have been called before hours input for " .. test_label)
    end
